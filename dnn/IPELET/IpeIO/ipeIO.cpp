...
bool Get_arcs(IpeletData* data, IpeletHelper* helper,
	std::vector<CurveSegment>& ret, bool applyTrans) {
	Page* page = data->iPage;
	int sel = page->primarySelection();
	if (sel < 0) {
	...
	for (int i = 0; i < n; i++) {
		...
		//insert arc into ret
		for (int j = 0; j < cv->countSegments(); j++) {
			if ((cv->segment(j)).type() != CurveSegment::EArc) continue;

			if (applyTrans) {
				ipe::Vector cp[2];
				ipe::Matrix ellM = applyTransformations(cv->segment(j).arc().iM, transM);
				cp[0] = applyTransformations(cv->segment(j).cp(0), transM);
				cp[1] = applyTransformations(cv->segment(j).cp(1), transM);
				Curve* tempCurve = new Curve();
				tempCurve->appendArc(ellM, cp[0], cp[1]);
				ret.push_back(tempCurve->segment(0));
			}
			else {
				ret.push_back(cv->segment(j));
	...
	if (ret.size() == 0) {
		helper->message("No arc selection");
		return false;
	}
	else return true;
}
...
bool IPEIO::Draw_arc(IpeletData* data, IpeletHelper* helper, const ipe::CurveSegment& cs) {
	Curve* sp = new Curve;
	sp->appendArc(cs.arc().iM, cs.cp(0), cs.cp(1));
	ipe::Shape shape;
	shape.appendSubPath(sp);
	Path* obj = new Path(data->iAttributes, shape);
	set_Attrs(obj);

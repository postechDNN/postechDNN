...
void AddDialog::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_TX, m_edit_tx);
	DDX_Control(pDX, IDC_EDIT_TY, m_edit_ty);
}
...
void AddDialog::OnBnClickedOk()
{
	...
	this->m_edit_tx.GetWindowTextW(temp);
	this->coordinate[2] = _wtof(temp);
	this->m_edit_ty.GetWindowTextW(temp);
	this->coordinate[3] = _wtof(temp);

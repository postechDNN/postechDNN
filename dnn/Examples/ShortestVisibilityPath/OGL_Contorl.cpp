...
void OGL_Contorl::OnPaint()
{
	...
	std::vector<std::pair<double, double>> inputPolygon = this->DDS.svp.getPolygonPoints();
	...
	std::vector<std::pair<double, double>> queryPoints = this->DDS.svp.getQueryPoints();
	if (this->DDS.object2D.getDrawVertices()) {
	...
	if (queryPoints.size() > 1) {
		...
		if (this->DDS.object2D.getDrawPath()) {
			...
			glLineWidth(3.0f);
			if (this->DDS.svp.getMode() == 0) {
				if (this->DDS.svp.minmaxStrings.size() > 0) {
					for (int j = 0; j < this->DDS.svp.minmaxStrings[0].size() - 1; j++) {
						OGL_Point sp(this->DDS.svp.minmaxStrings[0][j].first, this->DDS.svp.minmaxStrings[0][j].second);
						OGL_Point ep(this->DDS.svp.minmaxStrings[0][j + 1].first, this->DDS.svp.minmaxStrings[0][j + 1].second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
				...
				if (this->DDS.svp.minmaxStrings.size() > 1) {
					for (int j = 0; j < this->DDS.svp.minmaxStrings[1].size() - 1; j++) {
						OGL_Point sp(this->DDS.svp.minmaxStrings[1][j].first, this->DDS.svp.minmaxStrings[1][j].second);
						OGL_Point ep(this->DDS.svp.minmaxStrings[1][j + 1].first, this->DDS.svp.minmaxStrings[1][j + 1].second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
					...
					if (this->DDS.object2D.getDrawLine()) {
						::glColor3f(1.0f, 1.0f, 0.0f);
						glLineWidth(3.0f);
						OGL_Point sp(this->DDS.svp.minmaxStrings.front().back().first, this->DDS.svp.minmaxStrings.front().back().second);
						OGL_Point ep(this->DDS.svp.minmaxStrings.back().back().first, this->DDS.svp.minmaxStrings.back().back().second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
				...
				/*
				if (this->DDS.object2D.getDrawLine() && !this->DDS.svp.minmaxLine.empty()) {
					::glColor3f(1.0f, 1.0f, 0.0f);
					glLineWidth(3.0f);
					OGL_Point sp(this->DDS.svp.minmaxLine.front().first, this->DDS.svp.minmaxLine.front().second);
					OGL_Point ep(this->DDS.svp.minmaxLine.back().first, this->DDS.svp.minmaxLine.back().second);
					glBegin(GL_LINES);
					glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
					glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
					glEnd();
				...
				*/
				...
				if (this->DDS.svp.minsumStrings.size() > 0) {
					for (int j = 0; j < this->DDS.svp.minsumStrings[0].size() - 1; j++) {
						OGL_Point sp(this->DDS.svp.minsumStrings[0][j].first, this->DDS.svp.minsumStrings[0][j].second);
						OGL_Point ep(this->DDS.svp.minsumStrings[0][j + 1].first, this->DDS.svp.minsumStrings[0][j + 1].second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
				...
				if (this->DDS.svp.minsumStrings.size() > 1) {
					for (int j = 0; j < this->DDS.svp.minsumStrings[1].size() - 1; j++) {
						OGL_Point sp(this->DDS.svp.minsumStrings[1][j].first, this->DDS.svp.minsumStrings[1][j].second);
						OGL_Point ep(this->DDS.svp.minsumStrings[1][j + 1].first, this->DDS.svp.minsumStrings[1][j + 1].second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
					...
					if (this->DDS.object2D.getDrawLine()) {
						::glColor3f(1.0f, 1.0f, 0.0f);
						glLineWidth(3.0f);
						OGL_Point sp(this->DDS.svp.minsumStrings.front().back().first, this->DDS.svp.minsumStrings.front().back().second);
						OGL_Point ep(this->DDS.svp.minsumStrings.back().back().first, this->DDS.svp.minsumStrings.back().back().second);
						glBegin(GL_LINES);
						glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
						glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
						glEnd();
				...
				/*
				if (this->DDS.object2D.getDrawLine() && !this->DDS.svp.minsumLine.empty()) {
					::glColor3f(1.0f, 1.0f, 0.0f);
					glLineWidth(3.0f);
					OGL_Point sp(this->DDS.svp.minsumLine.front().first, this->DDS.svp.minsumLine.front().second);
					OGL_Point ep(this->DDS.svp.minsumLine.back().first, this->DDS.svp.minsumLine.back().second);
					glBegin(GL_LINES);
					glVertex2d((sp.getX() - normTrans[0]) / normMul[0], (sp.getY() - normTrans[1]) / normMul[1]);
					glVertex2d((ep.getX() - normTrans[0]) / normMul[0], (ep.getY() - normTrans[1]) / normMul[1]);
					glEnd();
...
void OGL_Contorl::setDrawObject(int m, OBJECT o, bool b) {
	if (m == 2 || m==3) {
		...
		case 4:
			this->DDS.object2D.setDrawLine(b);
		default:
			break;
		}
	}
...
void OGL_Contorl::addVertex(int x, int y) {
	this->DDS.svp.addVertex(x, y);
	Invalidate(TRUE);
	UpdateWindow();
}

bool OGL_Contorl::readPolygon(std::string fileName) {
	int readResult = this->DDS.svp.readPolygon(fileName);
	if (readResult == 0) {
		return false;
	}
	else {
		std::vector<std::pair<double, double>> polygon_vertices = this->DDS.svp.getPolygonPoints();
...
int OGL_Contorl::addQueryPoint(double x, double y) {
	...
	return this->DDS.svp.addQueryPoint(ax, ay);
	Invalidate(TRUE);
	UpdateWindow();
}

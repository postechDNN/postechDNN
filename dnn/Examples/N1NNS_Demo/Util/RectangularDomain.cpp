...
vector<std::pair<Point*,double>> RectangularDomain::kNNS(Point* query, int k)
{
	...
	if (!(notinterrect && notinterpoint)) {
		delete query;
		return vector<std::pair<Point*,double>>(0);
	}
	...
	vector<std::pair<Point*, double>> result;
	if (k == -1) {
		int resindex = 0;
		...
		result.push_back(std::make_pair(data[resindex],dist[resindex]));
		return result;
	}
	else {

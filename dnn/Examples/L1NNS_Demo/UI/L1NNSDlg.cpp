...
void CDNNDemoDlg::printLogEditBox() {
	int resultSize = this->m_picture_opengl.L1DS.getQueryResultSize();
	CString resultLog;
	for (int i = 0; i < resultSize; i++) {
		CString temp1, temp2;
		temp1.Format(_T("%d"), i);
		resultLog += temp1 + _T("-th neighbor: (");
		std::pair<double, double> neighbor = this->m_picture_opengl.L1DS.getQueryResultCoord(i);
		temp1.Format(_T("%lf"), neighbor.first);
		temp2.Format(_T("%lf"), neighbor.second);
		resultLog += temp1 + _T(", ") + temp2 + _T(")\r\n Geodesic distance: ");
		temp1.Format(_T("%lf"), this->m_picture_opengl.L1DS.getQueryResultDist(i));
		resultLog += temp1 + _T("\r\n");
	}
...
void CDNNDemoDlg::OnBnClickedButtonFile()
{
	...
	if (IDOK == dlg.DoModal()){
		this->m_picture_opengl.L1DS.clearAll();
	...
	if (menu != LB_ERR) {
		...
		if (IDOK == dlg.DoModal()) {
			if (this->m_check_drawing_polygon.GetCheck()) {
				if (this->m_picture_opengl.L1DS.insertObstacleVertex(dlg.coordinate[0], dlg.coordinate[1])) {
					CString temp;
					...
					MessageBox(_T("Rectangle input failed."), _T("Error"), MB_ICONHAND);
				}
			}
			else if (this->m_check_drawing_point.GetCheck()) {
				if (this->m_picture_opengl.L1DS.insertInputPoint(dlg.coordinate[0], dlg.coordinate[1])) {
					CString temp;
					temp.Format(_T("%lf"), dlg.coordinate[0]);
					this->m_edit_query_sx.SetWindowTextW(temp);
					temp.Format(_T("%lf"), dlg.coordinate[1]);
	...
	if (this->m_check_query1.GetCheck()) {
		...
		case 0:
			if (rect.left < point.x && point.x < rect.right &&
				...
				if (this->m_check_drawing_polygon.GetCheck()) {
					if (this->m_picture_opengl.addVertexPolygon(x, y)) {
						CString temp;
						temp.Format(_T("%d"), x);
						this->m_edit_query_sx.SetWindowTextW(temp);
						temp.Format(_T("%d"), y);
		...
		case 1:			
			if (rect.left < point.x && point.x < rect.right &&
				...
				if (this->m_check_drawing_polygon.GetCheck()) {
					if (m_picture_opengl.addVertexPolygonAlign(x, y)) {
						CString temp;
						temp.Format(_T("%lf"), this->m_picture_opengl.L1DS.lastInput[0]);
						this->m_edit_query_sx.SetWindowTextW(temp);
						temp.Format(_T("%lf"), this->m_picture_opengl.L1DS.lastInput[1]);
						this->m_edit_query_sy.SetWindowTextW(temp);
					}
					else {
						MessageBox(_T("Rectangle input failed."), _T("Error"), MB_ICONHAND);
...
void CDNNDemoDlg::OnClickedButtonSave()
{
	...
	if (IDOK == dlg.DoModal()) {
		...
		case 0: // input polygon
			m_picture_opengl.L1DS.printInputData(std::string(CT2CA(pathName)));
			break;
		case 1: // query
			// Query time
			m_picture_opengl.L1DS.printQueryResult(std::string(CT2CA(pathName)));
			break;
		default:
			break;
		}
...
void CDNNDemoDlg::OnClickedButtonDelete()
{
	...
	case 0: // input polygon
		if (this->m_check_drawing_polygon.GetCheck()) {
			this->m_picture_opengl.L1DS.deleteObstacle();
		}
		else if (this->m_check_drawing_point.GetCheck()) {
			this->m_picture_opengl.L1DS.deleteInputPoint();
		}
	...
	case 1: // query
		if (this->m_check_drawing_point.GetCheck()) {
			...
			this->m_picture_opengl.L1DS.deleteQueryPoint();
...
void CDNNDemoDlg::OnBnClickedCheckDrawingPolygon()
{
	if (this->m_check_drawing_polygon.GetCheck() == true) {
		this->m_picture_opengl.L1DS.clearObstacleInputTemp();
		this->m_picture_opengl.L1DS.clearQuery();
		this->m_check_drawing_point.SetCheck(false);
		this->m_check_drawing_query.SetCheck(false);
	}
	else {
...
void CDNNDemoDlg::OnBnClickedButtonKnn()
{
	...
	if (isInt) {
		this->m_picture_opengl.L1DS.setKNN(_ttoi(text));
	}
	else {

...
void OGL_Contorl::OnPaint()
{
	...
	this->L1DS.object2D.getNorm(normTrans, normMul, 2);
	...
	*/
	...
	// Obstacle temp
	if (this->L1DS.object2D.getDrawObject(VERTEX)) {
		if (this->L1DS.isObsTempInserted()) {
			::glColor3f(0.0f, 0.0f, 0.5f);
			glPointSize(5.0f);
			glBegin(GL_POINTS);
			auto pt = this->L1DS.getObsTemp();
	...
	if (this->L1DS.object2D.getDrawObject(EDGE)) {
		::glColor3f(1.0f, 1.0f, 1.0f);
		glLineWidth(3.0f);
		for (int i = 0; i < this->L1DS.getObstacleSize(); i++) {
			glBegin(GL_LINE_LOOP);
			for (int j = 0; j < 4; j++) {
				auto pt = this->L1DS.getObstacleVertex(i, j);
				OGL_Point p(pt.first, pt.second);
				glVertex2d((p.getX() - normTrans[0]) / normMul[0], (p.getY() - normTrans[1]) / normMul[1]);
			}
			glEnd();
	...
	// input points
	if (this->L1DS.object2D.getDrawObject(POINTSET)) {
		::glColor3f(1.0f, 1.0f, 1.0f);
		glPointSize(5.0f);
		glBegin(GL_POINTS);
		for (int i = 0; i < this->L1DS.getInputPointsSize(); i++) {
			auto pt = this->L1DS.getInputPoint(i);
			OGL_Point p(pt.first, pt.second);
			glVertex2d((p.getX() - normTrans[0]) / normMul[0], (p.getY() - normTrans[1]) / normMul[1]);
		}
		glEnd();
	...
	// query result
	if (this->L1DS.object2D.getDrawObject(QUERY)) {
		::glColor3f(0.0f, 1.0f, 0.0f);
		glPointSize(5.0f);
		glBegin(GL_POINTS);
		for (int i = 0; i < this->L1DS.getQueryResultSize(); i++) {
			auto pt = this->L1DS.getQueryResultCoord(i);
			OGL_Point p(pt.first, pt.second);
			glVertex2d((p.getX() - normTrans[0]) / normMul[0], (p.getY() - normTrans[1]) / normMul[1]);
		}
		glEnd();
	...
	// query point
	if (this->L1DS.object2D.getDrawObject(QUERY)) {
		if (this->L1DS.isQueryInserted()) {
			auto pt = this->L1DS.getQueryPoint();
...
void OGL_Contorl::setDrawObject(int m, OBJECT o, bool b) {
	if (m == 2) {
		this->L1DS.object2D.setDrawObject(o, b);
	}
	else if (m == 3) {
...
void OGL_Contorl::setCamera(int tx, int ty, int width, int height) {
	this->L1DS.object2D.setNorm(tx, ty, width, height);
}

int OGL_Contorl::addVertexPolygon(double x, double y) {
	return this->L1DS.insertObstacleVertex(x,y);
}

int OGL_Contorl::addVertexPolygonAlign(double x, double y) {
	int ax, ay;
	double normTrans[3];
	double normMul[3];
	this->L1DS.object2D.getNorm(normTrans, normMul, 2);
	...
	return this->L1DS.insertObstacleVertex(ax, ay);
}

int OGL_Contorl::addPoint(double x, double y) {
	return this->L1DS.insertInputPoint(x, y);
}
...
int OGL_Contorl::addPointAlign(double x, double y) {
	...
	return this->L1DS.insertInputPoint(ax, ay);
}

bool OGL_Contorl::readInputData(std::string fileName) {
	int readResult = this->L1DS.readInputData(fileName);
	if (readResult == 0) {
		return false;
	}
	else {
...
int OGL_Contorl::addQueryPoint(double x, double y) {
	return this->L1DS.insertQueryPoint(x, y);
}
...
int OGL_Contorl::addQueryPointAlign(double x, double y) {
	...
	return this->L1DS.insertQueryPoint(ax, ay);
}

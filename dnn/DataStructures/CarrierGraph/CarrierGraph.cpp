...
CarrierGraph::CarrierGraph(vector<Rect*> rset, vector<Point*> pset, Rect* bb, int _dir) {
	this->dir = _dir;
	...
	switch (dir) {
	case XPOS: CarrierGraphxpos(); break;
	case XNEG: CarrierGraphxneg(); break;
	case YPOS: CarrierGraphypos(); break;
	case YNEG: CarrierGraphyneg(); break;
	}
	vcnt = vlist.size();
	ecnt = elist.size();
...
int CarrierGraph::insertion(vector<Rect*> rins, vector<Point*> pins) {
	...
	for (int i = 0; i < pins.size(); i++) {
		bool notinterrect = true, notinterpoint = true;
		for (int j = 0; j < rectset.size(); j++) {
			if (intersect(rectset[j], pins[i])) {
				notinterrect = false;
				break;
			}
		}
		for (int j = 0; j < pointset.size(); j++) {
			if (*pins[i] == *pointset[j]) {
				notinterpoint = false;
				break;
			}
		}
	...
	if (successcnt > 0) {
		...
		vconnect.clear();
		for (int i = 0; i < rectset.size(); i++) {
			vlist.push_back(rectset[i]->getld());
			vlist.push_back(rectset[i]->getlu());
			vlist.push_back(rectset[i]->getrd());
			vlist.push_back(rectset[i]->getru());
		}
		for (int i = 0; i < rectset.size(); i++)
			pointset.push_back(pointset[i]);
		for (int i = 0; i < rectset.size(); i++) {
			elist.push_back(new Edge(rectset[i]->getld(), rectset[i]->getlu()));
			elist.push_back(new Edge(rectset[i]->getrd(), rectset[i]->getru()));
			elist.push_back(new Edge(rectset[i]->getld(), rectset[i]->getrd()));
			elist.push_back(new Edge(rectset[i]->getlu(), rectset[i]->getru()));
		}
...
int CarrierGraph::insertion(vector<Point*> pins) {
	return insertion(vector<Rect*>(), pins);
}

int CarrierGraph::insertion(vector<Rect*> rins) {

...
bool Edge::on(Point& p) {
	...
	double d = t_x - s_x;
	if (std::abs(d) < ERR) {
		if (std::abs(p_x - t_x) <= ERR && p_y <= max(t_y, s_y) && p_y >= min(t_y, s_y)) {
			return true;
		}
		else return false;
	}
		...
		if (abs(y - p_y) <= ERR && p_y <= max(t_y, s_y) && p_y >= min(t_y, s_y)) {
			return true;
		}
		else return false;
	}
	...
	if (std::abs(d) < ERR) {	//two line segment have same slope.
		if (std::abs(t) < ERR) {	//two line segment lies on same line.
			if (std::abs(x_1 - x_2) < ERR) {	//Vertical Line
				if (max(y_1, y_2) < min(y_3, y_4) || min(y_1, y_2) > max(y_3, y_4))	return nullptr;
				else if (!closed && (std::abs(max(y_1, y_2) - min(y_3, y_4)) < ERR || std::abs(min(y_1, y_2) - max(y_3, y_4)) < ERR)) return nullptr;
				else {	//Intersect!
				...
				if (max(x_1, x_2) < min(x_3, x_4) || min(x_1, x_2) > max(x_3, x_4))	return nullptr;
				else if (!closed && (std::abs(max(x_1, x_2) - min(x_3, x_4)) < ERR || std::abs(min(x_1, x_2) - max(x_3, x_4)) < ERR)) return nullptr;
				else {

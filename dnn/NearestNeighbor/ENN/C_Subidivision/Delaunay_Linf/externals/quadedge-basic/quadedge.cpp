...
// segments: NEED TO HAVE THE TYPE to draw using gnuplot later 
// edges: edge information using node indices
{
	if (Qedge()->TimeStamp(stamp)) {
		...
		printf("Edge: ([%d] %.1f, %.1f) to ([%d] %.1f, %.1f)\n", a.idx, a.x, a.y, b.idx, b.x, b.y);
		...
		std::pair<int, int> edge = {a.idx, b.idx}; 
		...
		// visit neighbors
		//printf("Draw Onext \n");
		Onext()->Draw(stamp, segments, edges);
		//printf("Draw Oprev \n");
		Oprev()->Draw(stamp, segments, edges);
		//printf("Draw Dnext \n");
		Dnext()->Draw(stamp, segments, edges);
		//printf("Draw Dprev \n");
		Dprev()->Draw(stamp, segments, edges);
